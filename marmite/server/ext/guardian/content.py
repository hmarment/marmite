...
def get_page(page_number, page_size=50):
    ...
    url = urlparse(BASE_URL + Endpoints.ListRecipes.value
                   .format(API_KEY=API_KEY, page=page_number,
                           page_size=page_size)).geturl()
...
def _recipe_publication_dates(recipe_jsons):
    ...
    return [recipe_json.get('webPublicationDate')
...
def _any_new_recipes(last_published_date, recipe_jsons):
    ...
    return any([publication_date > last_published_date
                for publication_date in recipe_dates])

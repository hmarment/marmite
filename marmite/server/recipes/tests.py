...
def create_recipe(name, short_description, type_, source, preparation_time, cooking_time, instructions, image):
...
def test_update_recipe(client, setup_test_recipe):
    ...
    response = client.put(reverse('recipe_detail_view', args=[recipe.id]), recipe_changes, content_type='application/json')
    ...
    assert updated_recipe_json.get('preparation_time') == recipe_changes.get('preparation_time')
